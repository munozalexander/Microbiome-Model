...
##### FUNCTIONALITY #####
def init_bacteria(c_section=False):
    ''' Initialize gut patch with some bacteria '''
    global tissue_patch
    percent_to_fill = init_percent_full/2 if c_section else init_percent_full
    ...
    for row_num in range(tissue_shape): #loop over tissue
        for col_num in range(tissue_shape):
            if random.random() < percent_to_fill:
                r = random.random()
...
if __name__ == "__main__":
    ...
    if homeostasis_flag == 0: #healthy
        ...
        for current_time in range(10):
            ...
            record_patch_state()
        fecal_transplant()
        for current_time in range(time_steps - 10):
            divide()
            death()
            record_patch_state()
    elif homeostasis_flag == 5: #Cesarean section
        init_bacteria(c_section=True)
        infect()
        record_patch_state()
        for current_time in range(time_steps):
            divide()
    ...
    if output_flag == 0: #print to console
        ...
        anim.save(fname)
        plt.show()
    elif output_flag == 2: #two static heatmaps for initial and end times
        if homeostasis_flag == 0:
            two_heatmaps()
        elif homeostasis_flag == 1:
