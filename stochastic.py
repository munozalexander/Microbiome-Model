...
output_flag:            select the desired output to be generated by the
                        ...
                        * 1 : heatmap at final timestep
                        * 2 : display animated heatmap
...
if __name__ == "__main__":
    ...
    if output_flag == 3:
        images = []
        print "HISTORY"
        for ts in tissue_patch_history:
            print_tissue(ts)
        for time_step in range(time_steps):
            fig = plt.figure()
            ax = fig.add_subplot(111)
            im = ax.matshow(tissue_patch_history[time_step])
            fname = 'static_' + str(time_step)
            fig.colorbar(im)
            fig.savefig(fname)
            images.append(Image.open(fname + '.png'))
        writeGif('automaton.gif', images, duration = .2)
